...
## В данном учебном проекте я реализовал статическую библиотеку string.h с дополнениями. Далее будет перечислено что реализовано в проекте.
...
### 2) sprintf, спецификаторы, ширина, длина
...
| 4 | # | При использовании со спецификаторами o, x или X перед числом вставляется 0, 0x или 0X соответственно (для значений, отличных от нуля).|
...
| № | .точность | Описание |
| --- | --- | --- |
| 1	| .число | Для целочисленных спецификаторов (d, i, o, u, x, X) − точность определяет минимальное количество записываемых цифр.
...
### 3) Специальные функции обработки строк (вдохновленные классом String в языке C#)
